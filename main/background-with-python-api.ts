...
ipcMain.on('getPythonPort', (event: any) => {
  ...
  superagent
    .get('http://127.0.0.1:' + pyPort + '/graphql/?query=%7Bawake%7D')
    .then(() => {
    ...
    .catch(() => {
      ...
      const exePath =
        process.platform === 'win32'
          ...
          : '/Users/vmiron/lessons/electron/my-app/pythondist/api';
      if (__dirname.indexOf('app.asar') > 0) {
        // dialog.showErrorBox("info", "packaged");
        if (existsSync(exePath)) {
          pyProc = execFile(
            exePath,
            ['--apiport', pyPort.toString()],
            () => {}
          );
          if (pyProc === undefined) {
        ...
        if (existsSync(srcPath)) {
          pyProc = require('cross-spawn')('python', [
            srcPath,
            '--apiport',
            pyPort.toString(),
          ]);
...
const exitPyProc = () => {
  ...
  //
  superagent
    .get('http://127.0.0.1:' + pyPort + '/graphql/?query=%7Bexit%7D')
    .then()
    .catch();
  pyProc = null;
};
